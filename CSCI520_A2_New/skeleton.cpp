	...
	for (int i = 1; (!done) && (i < MAX_BONES_IN_ASF_FILE); i++) {
		...
		while (1) {
			...
			if (strcmp(keyword, "direction") == 0)
				sscanf(str, "%s %lf %lf %lf", keyword, &m_pBoneList[i].dir[0],
						&m_pBoneList[i].dir[1], &m_pBoneList[i].dir[2]);
			...
			if (strcmp(keyword, "axis") == 0)
				sscanf(str, "%s %lf %lf %lf", keyword, &m_pBoneList[i].axis_x,
						&m_pBoneList[i].axis_y, &m_pBoneList[i].axis_z);
			...
			if (strcmp(keyword, "dof") == 0) {
				...
				while (token != NULL) {
					...
					if (strcmp(token, "rx") == 0) {
						m_pBoneList[i].dofrx = 1;
						m_pBoneList[i].dofo[tdof] = 1;
					} else if (strcmp(token, "ry") == 0) {
						m_pBoneList[i].dofry = 1;
						m_pBoneList[i].dofo[tdof] = 2;
					} else if (strcmp(token, "rz") == 0) {
						m_pBoneList[i].dofrz = 1;
						m_pBoneList[i].dofo[tdof] = 3;
					} else if (strcmp(token, "tx") == 0) {
						m_pBoneList[i].doftx = 1;
						m_pBoneList[i].dofo[tdof] = 4;
					} else if (strcmp(token, "ty") == 0) {
						m_pBoneList[i].dofty = 1;
						m_pBoneList[i].dofo[tdof] = 5;
					} else if (strcmp(token, "tz") == 0) {
						m_pBoneList[i].doftz = 1;
						m_pBoneList[i].dofo[tdof] = 6;
					} else if (strcmp(token, "l") == 0) {
						m_pBoneList[i].doftl = 1;
						m_pBoneList[i].dofo[tdof] = 7;
		...
		if ((!m_pBoneList[i].dofrx) && (!m_pBoneList[i].dofry)
				&& (!m_pBoneList[i].dofrz))
			MOV_BONES_IN_ASF_FILE -= 1;
		...
		if (strcmp(keyword, "end") == 0)
			...
			while (part_name != NULL) {
				if (j == 0)
					...
					setChildrenAndSibling(parent,
							&m_pBoneList[name2idx(part_name)]);
...
void Skeleton::RotateBoneDirToLocalCoordSystem() {
	...
	for (i = 1; i < NUM_BONES_IN_ASF_FILE; i++) {
		...
		vector_rotationXYZ(&m_pBoneList[i].dir[0], -m_pBoneList[i].axis_x,
				-m_pBoneList[i].axis_y, -m_pBoneList[i].axis_z);
	}
}
